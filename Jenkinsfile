        ...
        stage("Deploy") {
            steps {
                sshagent(['vultr-ssh-key']) {
                    ...
                    sh '"echo $DOCKERHUB_CREDENTIALS_PSW | docker login -u $DOCKERHUB_CREDENTIALS_USR --password-stdin && docker pull ridwan779/ip-location && docker run --name ip-location -p 8881:8024 -d ridwan779/ip-location && exit"'
                }
            }
        }
